...
export function AddTransactionDialog({
  ...
  function handleLinkGroupExpense(expenseId: string) {
    ...
    if (!exp) return
    const splitAmt = exp.expense_splits[0]?.amount ?? 0
    setForm((p) => ({
      ...p,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="left-0 right-0 translate-x-0 w-full max-w-none bottom-0 top-auto sm:inset-auto sm:left-1/2 sm:right-auto sm:-translate-x-1/2 sm:top-1/2 sm:-translate-y-1/2 sm:max-w-md p-0 gap-0 max-h-[85svh] sm:max-h-[90vh] flex flex-col rounded-t-2xl sm:rounded-lg">
        {/* Sticky Header */}
